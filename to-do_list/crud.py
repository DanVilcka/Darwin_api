...
def get_items_by_user(db: Session, user_id: int):
    return (
        db.query(models.Privilege, models.Item)
        .join(models.Item, models.Privilege.user_id == models.Item.owner_id)
        .filter(models.Privilege.user_id == user_id, models.Item.owner_id == user_id)
        .all()
    )
...
def create_privelige(
    db: Session, privelige: schemas.PriveligeCreate, user_id: int, item_id: int
):
    db_privelige = models.Privelige(
        **privelige.model_dump(), item_id=item_id, user_id=user_id
    )
    db.add(db_privelige)
    db.commit()
...
def get_privelige_by_item_id(db: Session, item_id: int):
    return (
        db.query(models.Privelige).filter(models.Privelige.item_id == item_id).first()
    )
